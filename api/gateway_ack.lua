...
local DEFAULT_ERROR_PAGES = {
    -- NO_RULE: When no routing rules are configured (Orange theme)
    ...
    -- CONF_MISMATCH: When configuration doesn't match expected format (Red theme)
    ...
    -- NO_SERVER: When no server configuration is found for domain (Purple theme)
    ...
    if ruleFromRedis ~= nil and type(ruleFromRedis) ~= "userdata" then
        ...
        if ruleFromRedis.match and ruleFromRedis.match.rules then
            ...
            local results = gatewayHostRulesParser(ruleFromRedis.match.rules, ruleFromRedis.id, ruleFromRedis.priority,
                ...
                ruleFromRedis.match.response.consul_domain_name)
            return results
        end
    end
end
