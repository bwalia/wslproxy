...
# The WAF Efficacy Lab is a static page generated by gen_waf_lab.py and shipped
# next to this file in the ConfigMap. Serving it here (same-origin at /lab) is
# what lets a browser read this host's WAF blocks and x-waf-* headers. In the pod
# app.py and lab.html share /app; in the repo lab.html is one dir up from app/.
_HERE = os.path.dirname(os.path.abspath(__file__))
LAB_CANDIDATES = [os.path.join(_HERE, "lab.html"),
                  os.path.join(os.path.dirname(_HERE), "lab.html")]
...
def read_lab():
    for path in LAB_CANDIDATES:
        try:
            ...
            continue
    return page(
            "<h1>WAF Efficacy Lab</h1><p class=bad>lab.html is not deployed next to "
            "this app.</p><p>Add it to the <code>payments-api-src</code> ConfigMap "
            "(<code>--from-file=lab.html=lab.html</code>) — see the demo README.</p>", 200)


LANDING = page(f"""
<div style="margin:0 0 1.3rem;padding:1rem 1.2rem;border:1px solid #a5b4fc;border-radius:12px;background:#eef2ff">
🧪 <b>WAF Efficacy Lab</b> — fire every WAF rule (GET &amp; POST) at this host and watch, live,
what the WAF blocks. <a href="/lab" style="font-weight:600">Open the lab →</a>
</div>
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p in ("/lab", "/lab.html"):     return self._send(*read_lab())
