...
23. **cjson turns an empty `[]` into `{}` on any read-modify-write.** A plain `cjson.decode` gives an empty Lua table, which re-encodes as `{}`; `repo/codec.lua` also rebuilt every table on save and dropped metatables. Every backend rewrite of a WAF policy changed `ipLists.deny: []` to `{}`. Fixed for the storage path: `ConfigIO.decode(..., { preserve_arrays = true })` (used by `disk_driver`) decodes with a private cjson instance that keeps `cjson.array_mt`, and the codec copies keep the source metatable. The request path and `Helper.GetPayloads` still use plain cjson, so a *request body's* `[]` can still land as `{}`. Build new list fields in responses with `setmetatable({}, cjson.array_mt)`.
