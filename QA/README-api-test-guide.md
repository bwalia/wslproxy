...
- `FRONT_URL`: The front url of the environment.
...
You can trigger the test execution using GitHub Actions by following this [workflow_link](https://github.com/bwalia/wslproxy/actions/workflows/wslproxy-api-test.yml). Use the `workflow_dispatch` event to manually trigger the workflow. Before execution, select the target environment and choose whether to delete the rules after execution based on your test preferences. Then, click "Run workflow."
...
- int : http://api-int.wslproxy.com/
- int2 : http://api.int2.wslproxy.com/
...
- test2 : http://api.test2.wslproxy.com/
- test6 : http://api.test6.wslproxy.com/
