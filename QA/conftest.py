...
@pytest.fixture()
def tempDir_setup(request, tmpdir_factory): 

    chrome_driver_path = chromedriver_autoinstaller.install()
    chrome_service: Service = Service(executable_path=chrome_driver_path)
    tempDir=  tmpdir_factory.mktemp('downloads')
    ...
    for option in options:
        chrome_options.add_argument(option)
    preferences = {
        "download.default_directory": str(tempDir),
    ...
    request.cls.driver = driver
    request.cls.tempDir = tempDir

    yield driver, tempDir
