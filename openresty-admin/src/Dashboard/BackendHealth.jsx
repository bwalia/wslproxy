...
// ─── Compact StatCard (same pattern as IngressOverview) ─────────────────────
...
// ─── Chart Card wrapper (same pattern as IngressOverview) ───────────────────
...
const RuleHealthCard = ({ rule, formatNumber, formatBytes }) => {
  const theme = useTheme();
...
const BackendHealth = ({ healthData, topologyData, onRefresh, formatNumber, formatBytes }) => {
  const theme = useTheme();
  const { mode } = useThemeMode();
  const isDark = mode === "dark";
  ...
  const totalBackends = mergedRules.reduce((s, r) => s + r.backends.length, 0);
  const healthyBackends = mergedRules.reduce((s, r) => s + r.backends.filter((b) => b.healthy).length, 0);
  const totalRequests = mergedRules.reduce((s, r) => s + r.backends.reduce((bs, b) => bs + (b.stats?.requests || 0), 0), 0);
  const totalErrors = mergedRules.reduce((s, r) => s + r.backends.reduce((bs, b) => bs + (b.stats?.errors || 0), 0), 0);
  const errorRate = totalRequests > 0 ? ((totalErrors / totalRequests) * 100).toFixed(1) : "0.0";

  // Empty state
  if (mergedRules.length === 0) {
    return (
      <ChartCard title="Backend Health & Traffic" subtitle="Multi-backend routing overview" onRefresh={onRefresh} accentColor="#10b981">
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr 1fr", md: "repeat(4, 1fr)" }, gap: 2, mb: 3 }}>
        <StatCard title="Rules with Backends" value={totalRules} icon={AccountTreeIcon} color="#6366f1" />
        <StatCard title="Total Backends" value={totalBackends} icon={DnsRoundedIcon} color="#06b6d4" />
        <StatCard
      ...
      {mergedRules.map((rule) => (
        <RuleHealthCard key={rule.rule_id} rule={rule} formatNumber={formatNumber} formatBytes={formatBytes} />
      ))}
    </ChartCard>
  );
};
